...
class QuranService {
  ...
  static Future<Map<String, dynamic>> getSurahWithTranslation(
    int surahNumber,
  ) async {
    try {
      // Get Arabic text
      ...
      final translationResponse = await http.get(
        Uri.parse('$_baseUrl/surah/$surahNumber/id.indonesian'),
      );

      if (arabicResponse.statusCode == 200 &&
          translationResponse.statusCode == 200) {
        final Map<String, dynamic> arabicData = json.decode(
          arabicResponse.body,
        );
        final Map<String, dynamic> translationData = json.decode(
